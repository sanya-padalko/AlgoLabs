...
def plot_sorting_times():
    n_values = np.arange(0, 1050, 50)
    
    algorithms = {
        'Shell Sort': 'shell',
        'Original Shell Sort': 'orig-shell',
        'Pratt\'s Shell Sort': 'pratt-shell',
        'Ciura\'s Shell Sort': 'ciura-shell',
        'Gonnet\'s Shell Sort': 'gonnet-shell'
    }
    ...
    plt.title('Сравнение времени выполнения алгоритмов сортировки Шелла', 
              fontsize=14, fontweight='bold')
    plt.grid(True, alpha=0.3)
    plt.legend(loc='upper left', fontsize=10)
    
    plt.text(0.20, 0.98, 'Количество элементов: от 0 до 1000 (шаг 50)',
             transform=plt.gca().transAxes, fontsize=9, 
             verticalalignment='top', bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.5))
    
    plt.tight_layout()

    plt.savefig('1-shell-graph.png', dpi=300, bbox_inches='tight')
    print("График сохранен как '1-shell-graph.png'")
